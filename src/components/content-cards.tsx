...
import { MapPin, CalendarDays, Instagram, Camera } from "lucide-react";
  ...
  return (
    <figure className="card-lift group overflow-hidden rounded-3xl bg-card shadow-[var(--shadow-soft)]">
      ...
      <figcaption className="space-y-1.5 p-5">
        ...
        {(photo.photographer_name || photo.photographer_instagram) && (
          <div className="flex items-center gap-1.5 border-t border-border/60 pt-2 text-xs text-muted-foreground">
            ...
            {photo.photographer_instagram && (
              <a
                href={`https://instagram.com/${photo.photographer_instagram.replace(/^@/, "")}`}
                target="_blank"
              ...
              >
                ...
                {photo.photographer_instagram.replace(/^@/, "")}
              </a>
            )}
          </div>
        )}
